...
/**
 * Nitro bundles `@electric-sql/pglite` into `_libs/` but not the WASM payload
 * it reads at runtime (`pglite.data`). Local `vite preview` (no DATABASE_URL)
 * needs those files next to the chunk; deployed Neon never loads PGLite.
 */
function copyPgliteWasm() {
